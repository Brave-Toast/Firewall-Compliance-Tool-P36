"""This module provides functions to normalize firewall rules."""
...
def normalize_rules(rules: List[FirewallRule]) -> List[FirewallRule]:
    """Normalize a list of firewall rules by cleaning zones and addresses."""
    normalized = []
    for r in rules:
        ...
        r.destination_addresses = [
            addr.strip().lower() for addr in r.destination_addresses
            if addr.strip()
        ]
        if not r.name:
            r.name = r.id
